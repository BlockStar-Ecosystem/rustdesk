...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in BlockStarDesk
            // 2. Move the physical mouse
            // 3. Press "V" key in BlockStarDesk
            ...
            // we consider only BlockStarDesk operates the controlled side.
            //
...
fn key_sleep() {
    ...
    // Though `/Applications/BlockStarDesk.app/Contents/MacOS/rustdesk --server` in terminal is ok.
    let now = Instant::now();
